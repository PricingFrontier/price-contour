...
pub fn update_lambdas_subgradient(
    ...
    for (k, spec) in specs.iter().enumerate() {
        ...
        // quote or ~500 per quote.
        let threshold_scale = spec.threshold.abs().max(1.0);
...
mod tests {
    ...
    fn test_diminishing_step_size() {
        ...
        update_lambdas_subgradient(&mut lambdas_early, &specs, &totals, &default_scales(1), 0);

        let mut lambdas_late = vec![0.0];
        update_lambdas_subgradient(&mut lambdas_late, &specs, &totals, &default_scales(1), 99);
    ...
    fn test_scale_factor_amplifies() {
        ...
        update_lambdas_subgradient(&mut lambdas_scaled, &specs, &totals, &[10.0], 0);

        let mut lambdas_unscaled = vec![0.0];
        update_lambdas_subgradient(&mut lambdas_unscaled, &specs, &totals, &[1.0], 0);
